...
bool initDreamEngine(char *window_name, int resX_arg, int resY_arg,
                     bool fullscreen) {
  bool success = false;
  ...
  if (SDL_Init(SDL_INIT_EVERYTHING) == 0) {
    TTF_Init();
    window = SDL_CreateWindow((char *)"Hello", SDL_WINDOWPOS_UNDEFINED,
  ...
  for (int i = 0; i < samples; i++) {
    ...
    for (int ch = 0; ch < 4; ch++) {
      if (channels[ch].active && now < channels[ch].end_time) {
        ...
        mixed_sample +=
            ((channels[ch].sample_count % period) < (period / 2) ? 1 : -1) *
            4000;
...
void resetTimer(Timer *timer) { timer->current_time = 0; }

void incrementTimer(Timer *timer) { timer->current_time++; }

void setMaxTimer(Timer *timer, int time) { timer->max_time = time; }
